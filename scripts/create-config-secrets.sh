...
create_update_secret() {
   ...
   echo -e "\n${PURPLE}🚀 Creating/updating Secret for ${name}...${NC}"
   ...
   if aws secretsmanager create-secret \
      ...
      echo -e "${GREEN}✅ Secret '${BOLD}$SECRET_NAME_PREFIX/$name${NC}${GREEN}' created successfully!${NC}"
    else
      ...
      if aws secretsmanager update-secret \
         ...
         --secret-string file://"$file" \
         --region $AWS_REGION >/dev/null 2>&1; then
         echo -e "${GREEN}✅ Secret '${BOLD}$SECRET_NAME_PREFIX/$name${NC}${GREEN}' updated successfully!${NC}"
      else
